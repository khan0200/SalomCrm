...
<template>
  <div
    class="canvas-a4-page-container flex flex-col items-center select-none my-6 transition-all duration-200"
    :data-page-index="pageIndex"
    @pointerdown="emit('set-active-page', pageIndex)"
  >
    <!-- Page Header Label & Dimensions -->
    <div
      class="page-meta-header flex items-center justify-between pb-2 mb-2 text-zinc-400 font-serif text-[11px] select-none transition-colors"
      :style="{ width: `${scaledPageWidthPx + (showRulers ? 20 : 0)}px` }"
    >
      <!-- Left: Page Number and Active Badge -->
      <div class="flex items-center gap-2">
        <span
          class="w-2.5 h-2.5 rounded-full inline-block transition-all"
        ...
        >
          PAGE {{ page.pageNumber }} / {{ totalPages }} — A4 SHEET
          <span
            v-if="isActivePage"
            class="ml-1.5 px-1.5 py-0.5 rounded text-[9px] bg-blue-100 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 font-sans uppercase font-bold"
          >
            Faol
          </span>
        </span>
      ...
      <!-- Center: Duplicate & Delete Page Actions -->
      <div class="flex items-center gap-1 bg-white/95 dark:bg-zinc-800/95 backdrop-blur-xs border border-zinc-200/90 dark:border-zinc-700/80 rounded-xl px-1.5 py-0.5 shadow-2xs font-sans">
        <button
          type="button"
          class="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-zinc-700 active:scale-95 transition-all cursor-pointer select-none"
          title="Sahifani dublikat qilish (nusxalash)"
        ...
        <button
          type="button"
          class="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-zinc-700 active:scale-95 transition-all cursor-pointer select-none disabled:opacity-40 disabled:cursor-not-allowed"
          :disabled="totalPages <= 1"
          :title="totalPages <= 1 ? 'Kamida 1 ta sahifa bo\'lishi shart' : 'Sahifani o\'chirib yuborish'"
      ...
      <!-- Right: Page Margins and Dimensions -->
      <div class="flex items-center gap-2 font-mono text-[10px] text-zinc-400">
        <span>Margins: L{{ margins.left }} R{{ margins.right }} T{{ margins.top }} B{{ margins.bottom }}mm</span>
        <span>|</span>
    ...
    <div class="flex items-start">
      ...
      <div v-if="showRulers" class="flex flex-col shrink-0">
      ...
      <div class="flex flex-col">
        ...
        >
          ...
          <CanvasSmartGuides :guides="activeGuides" :zoom-level="zoomLevel" />
